...
<!-- 조회결과 반환 -->
<% List<Musical> mcLikeList = (List) request.getAttribute("mcLikeList"); %>
<!-- paging 반환 -->
...
$(document).ready(function(){
	...
	$(".dropdown").on("hide.bs.dropdown", function(){
		$("#dropdownMenu1").html('인기보기 <span class="glyphicon glyphicon-chevron-down"></span>');
	});
	$(".dropdown").on("show.bs.dropdown", function(){
		$("#dropdownMenu1").html('인기보기 <span class="glyphicon glyphicon-chevron-up"></span>');
	});
...
<div class="text-center">
	...
	<div class="dropdown" style="display:inline-block;">
  		<button onclick="handleTitleClick()" class="btn btn-default.focus dropdown-toggle btn-lg" type="button" id="dropdownMenu1" data-toggle="dropdown" aria-expanded="true">
    		인기보기 <span id="icon" class="glyphicon glyphicon-chevron-down" aria-hidden="true"></span>
	  	</button>
...
<c:forEach var="musical" items="${mcLikeList}"> 
...
<div class="text-center">
	<ul class="pagination">
		...
		<li><a href="./mcLike">&laquo;</a></li>
		<%	} %>

	
		<%-- 이전 페이지로 이동 --%>
		<%	if(paging.getCurPage() > 1) { %>
		<li><a href="./mcLike?curPage=<%=paging.getCurPage()-1 %>">&lt;</a></li>
		<%	} %>
		...
		<li class="active"><a href="./mcLike?curPage=<%=i %>"><%=i %></a></li>
		<%		} else { %>
		<li><a href="./mcLike?curPage=<%=i %>"><%=i %></a></li>
		<%		} %>
		<%	} %>
		...
		<%-- 마지막 페이지 이동 --%>
		<%	if(paging.getCurPage() != paging.getTotalPage()) { %>
		<li><a href="./mcLike?curPage=<%=paging.getTotalPage() %>">&raquo;</a></li>
		<%	} %>
